...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_size', type=int, default=300, help="number of times to generate from a batch", required=False)
    ...
    parser.add_argument('--n_props', type=int, default = 2, help="dimension of desired properties", required=False)
    parser.add_argument('--vocab_size', type=int, default = 111, help="size of vocabs", required=False)  # 97 including padding code "<"
    parser.add_argument('--block_size', type=int, default = 620, help="size of slices", required=False)   # 360 for slices with atom number smaller than 10.

    parser.add_argument('--prop_targets', type=parse_prop_condition, required=True,
                        help='Property condition as a string representation of a list of lists, e.g., "[[100.0,40.0,30.0],[400.0,200.0,100.0],[50.0,36.0,40.0]]"')
    ...
    if prop_condition is not None:
        for c in prop_condition:
            for i in tqdm(range(gen_iter)):
                ...
                y = model.sample(x, args.block_size, temperature=1.2, do_sample=True, top_k=0.0, top_p=0.9, prop=p)  
                for gen_mol in y:
                    completion = " ".join([itos[int(i)] for i in gen_mol])
                    completion = completion.replace("<", "").strip()
                    #if check_SLICES(completion, 4):
                    all_slices.append({'Bulk': c[0],'Shear': c[1],'SLICES': completion})
                    #else:
                        #pass
