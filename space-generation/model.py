...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.head_1 = nn.Linear(config.n_embd, config.vocab_size, bias=False)  ###回归头
        self.head_2 = nn.Linear(config.vocab_size, config.num_classes, bias=False)  ###分类头
    ...
    def forward(self, idx, targets=None, crystal_system=None):
        b, t = idx.size()
    ...
    @torch.no_grad()
    def sample(self, x, steps, temperature=1.0, do_sample=False, top_k=None, top_p=None, crystal=None):
        """
        ...
        for k in range(steps):
            ...
            logits, _, _, _ = self(x_cond, crystal_system = crystal) # for sampling, no target
