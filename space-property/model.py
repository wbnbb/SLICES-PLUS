...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.crystal_system_emb = nn.Embedding(config.num_classes, config.n_embd)  # 新增的嵌入层
        ...
        self.head = nn.Linear(config.n_embd, config.vocab_size, bias=False)  ###回归头
    ...
    def forward(self, idx, targets=None, prop = None, crystal_system=None):
        b, t = idx.size()
        ...
        crystal_system_embeddings = self.crystal_system_emb(crystal_system).expand(b, t, -1)  # 新增的嵌入
        x = self.drop(token_embeddings + position_embeddings + type_embeddings + crystal_system_embeddings)
        
        type_crystal = self.type_emb(torch.full((b, 1), 2, dtype=torch.long, device=idx.device))
        c = self.crystal_system_emb(crystal_system)
        c += type_crystal
        x_2 = torch.cat([c, x], 1)
        ...
        #crystal_system_logits = self.head_2(logits)

        # if we are given some desired targets also calculate the loss
        loss = None
        if targets is not None:
            regression_loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), targets.view(-1))
            #crystal_system = crystal_system.expand(b, t)
            #classification_loss = F.cross_entropy(crystal_system_logits.reshape(-1, crystal_system_logits.size(-1)), crystal_system.reshape(-1))  # 假设 crystal_system_logits 是模型预测的晶系种类
    ...
    @torch.no_grad()
    def sample(self, x, steps, temperature=1.0, do_sample=False, top_k=None, top_p=None, prop=None, crystal=None):
        """
        ...
        for k in range(steps):
            ...
            logits, _, _, _ = self(x_cond, prop = prop, crystal_system = crystal) # for sampling, no target
