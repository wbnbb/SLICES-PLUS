...
class InvCryRep:
    ...
    def check_SLICES(self,SLICES,strategy=3,dupli_check=True,graph_rank_check=True):
        ...
        """
        slices_parts = SLICES.split() 
        start_index = None
        for i, part in enumerate(slices_parts):
            if re.match(r'^[a-np-z]+$', part):  # 仅包含小写字母，且不包含'o'
                start_index = i
                break
        if start_index is None:
            SLICES = SLICES
        else:
            SLICES = ' '.join(slices_parts[:start_index])
    ...
    def check_SLICES_2(self, SLICES):
        slices_parts = SLICES.split() 
        start_index = None
        for i, part in enumerate(slices_parts):
            if re.match(r'^[a-np-z]+$', part):  # 仅包含小写字母，且不包含'o'
        ...
        if start_index is None:
            return False
        second_part = ''.join(slices_parts[start_index:]).replace(" ", "")
        pattern = r'^((?:[a-np-z]|o1){3}(?:\d+s){3})+$'
        if not re.match(pattern, second_part):
            return False
    ...
    def check_slices_full(self,SLICES,strategy=3,dupli_check=True,graph_rank_check=True):
        check_SLICES_result = self.check_SLICES(SLICES, strategy, dupli_check, graph_rank_check)
        check_SLICES_2_result = self.check_SLICES_2(SLICES)
    
        return check_SLICES_result and check_SLICES_2_result
    ...
    def structure2SLICES2(self, cif_path):
        ...
        def closest_key(value, key_dict):
            closest = None
            min_diff = float('inf')
            for key in key_dict.keys():
                try:
                    key_value = float(fractions.Fraction(key))
                except ValueError:
                    key_value = float(key)
                diff = abs(key_value - value)
                if diff < min_diff:
                    min_diff = diff
                    closest = key
            return closest
        ...
        for op in symmetry_operations:
            matrix = op.rotation_matrix  
            vector = op.translation_vector
            encoded_rows = []
            for row in matrix:  
                encoded_row = rotation_letter[str([int(round(val)) for val in row])]
                encoded_rows.append(encoded_row)

            encoded_vector = []
            for component in vector:
                closest = closest_key(component, translation_letter)
                encoded_component = translation_letter[closest]
                encoded_vector.append(encoded_component)
            
            combined_encoded_list = encoded_rows + encoded_vector
            encoded_matrix = ' '.join(combined_encoded_list)  
            encoded_matrices.append(encoded_matrix)
            
        slices_2 = ' '.join(encoded_matrices) 

        slices_2_list = slices_2.split(' ')
        grouped_slices = [' '.join(slices_2_list[i:i+6]) for i in range(0, len(slices_2_list), 6)]
        unique_slices = list(dict.fromkeys(grouped_slices))
        slices_2 = ' '.join(unique_slices)
